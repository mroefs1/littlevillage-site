...
- **The three Services & Support cards have no photos**, so they render striped placeholders. Therapeutic Services, Related Services and CPSE Evaluations each need one image; the field is waiting on each `page` document, so it is one upload apiece with no deploy.
