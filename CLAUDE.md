...
- [ ] **11.2 - Header, utility bar, footer (global chrome)** - utility bar, logo badge, nav link colors, Request Info/Donate pills, footer, restyled with new tokens. Affects every page at once. Verify: nav dropdowns (Programs/About/Current Families) and the mobile hamburger nav still function identically - appearance only should change.

- [ ] **11.3 - Homepage** - hero, stat strip, age-picker cards, enrollment steps, news/events row, "Already part of Little Village" band, donate banner. Verify: hero gallery autoplay, hover-pause, manual prev/next arrows, and `prefers-reduced-motion` behavior all still work - highest-risk component on this page.

- [ ] **11.4 - Admissions** - eligibility checklist, 4-step journey, $0 callout, FAQ accordion, closing CTA. Verify: FAQ accordion expand/collapse and `aria-expanded` states still function correctly.
